...
namespace MedianFilter
{
    public class Filter
    {
        ...
        public static void ParForGPU(ushort[] output, ushort[] input, int width, int height, int chunk)
        {
            ...
            for (int blockJ = blockIdx.y * chunk; blockJ < height; blockJ += gridDim.y * chunk)
            {
                for (int blockI = threadIdx.x + blockDim.x * blockIdx.x; blockI < width; blockI += blockDim.x * gridDim.x)
                {
                    // preload window
                    ...
                    for (int lj = 0; lj < chunk; ++lj)
                    {
                        ...
                        // process buffer -- extract the median.
